...
<task type="auto">
  ...
  <files>src/intake/gmail-monitor.ts, src/intake/finmo-docs.ts, src/intake/__tests__/finmo-docs.test.ts</files>
  <action>
...
Use `.js` extensions for all local imports. Follow the patterns from `src/webhook/server.ts` and `src/webhook/queue.ts`.
  ...
  <verify>Run `npx vitest run src/intake/__tests__/finmo-docs.test.ts` and confirm all tests pass. Run `npx tsc --noEmit` with no errors.</verify>
  <done>Gmail monitor scheduler created with upsertJobScheduler at configurable interval. Finmo document handler accepts resthook events and enqueues to intake queue with dedup. Queue follows same lazy singleton pattern as webhook queue. Tests verify handler responses, payload extraction, and dedup jobId.</done>
</task>

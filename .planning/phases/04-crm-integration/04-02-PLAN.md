...
2. **DEV MODE:** If `crmConfig.isDev`, prefix `firstName` with `[TEST]` using `devPrefix()` so test contacts are visually distinct and filterable in MyBrokerPro. Example: "[TEST] John Smith"
3. Pass `email`, `firstName` (with dev prefix if applicable), `lastName`, `phone` (if provided)
4. Pass `customFields` array in the correct format: `[{ id, field_value }]`
5. CRITICAL: Do NOT include `EXISTING_FIELDS.FINMO_DEAL_ID`, `EXISTING_FIELDS.FINMO_APPLICATION_ID`, or `EXISTING_FIELDS.FINMO_DEAL_LINK` in the customFields update — these are managed by Finmo's own sync. Only include doc tracking fields.
6. Return the contact ID and whether it was newly created
...
```
Use `devPrefix()` from config so dev tasks show as `[TEST] Review doc request — John Smith`.
...
```
Use `devPrefix()` so dev tasks show as `[TEST] PRE docs complete — John Smith`.
...
```
Use `devPrefix()` so dev opportunities show as `[TEST] John Smith — Doc Collection`.

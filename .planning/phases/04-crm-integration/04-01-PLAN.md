...
```
# Environment Mode
# development = [TEST] prefix on contacts/tasks, verbose logging, emails blocked
...
# Document Intake Email (dev uses dev@, production uses docs@)
DOC_INBOX=dev@venturemortgages.com
# production: docs@venturemortgages.co
```
...
export interface CrmConfig {
  appEnv: AppEnv;
  isDev: boolean;  // convenience: appEnv === 'development'
  apiKey: string;
  baseUrl: string;
  locationId: string;
  docInbox: string;  // dev@venturemortgages.com or docs@venturemortgages.co
  userIds: {
    cat: string;
    taylor: string;
  };
...
Also add a `devPrefix` utility function to the config module:
```typescript
/** In dev mode, prefixes strings with [TEST] so they're visible and filterable in CRM */
export function devPrefix(text: string): string {
  return crmConfig.isDev ? `[TEST] ${text}` : text;
}
```

The config reads `APP_ENV` from `.env`, defaulting to `'development'` if not set (safe default — dev mode unless explicitly set to production). `isDev` is derived from `appEnv`.
```

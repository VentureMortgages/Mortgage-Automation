...
3. Update `validateConfig()` to also check `opportunityFieldIds` (same pattern as existing `fieldIds` validation, using `oppFieldIdToEnvKey` helper).

Do NOT remove existing contact-level `fieldIds` — they remain for backward compatibility during migration. The contact fields will be deprecated in a later phase.
  </action>
